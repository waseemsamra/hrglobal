...
export default function IndustriesManager() {
  ...
  async function call(method, body, qs = "") {
    setError("");
    const res = await fetch(`/api/industries${qs}`, {
      method,
  ...
  return (
    <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-stack-lg">
      <h3 className="text-title-md font-title-md mb-stack-md">Industries</h3>
      ...
      {/* Add industry */}
      <form onSubmit={addIndustry} className="flex flex-wrap items-end gap-stack-sm mb-stack-md">
        <div className="space-y-base flex-1 min-w-[180px]">
          <label className="text-label-md font-label-md text-on-surface-variant">INDUSTRY NAME</label>
          <input
      ...
      <div className="border border-outline-variant rounded-xl divide-y divide-outline-variant overflow-hidden">
        {industries.length === 0 && (
          <p className="p-4 text-body-sm text-on-surface-variant">No industries yet.</p>
        )}
        {industries.map((ind) => (
          <div key={ind.id} className="flex items-center gap-2 px-4 py-3 hover:bg-surface-container-low transition-colors">
            {editing?.id === ind.id ? (
              ...
              <>
                <div className="flex-1">
                  <p className="font-body-md font-bold">{ind.name}</p>
                  {ind.description && (
                    <p className="font-body-sm text-on-surface-variant">{ind.description}</p>
                  )}
                </div>
                <button
                  onClick={() => setEditing({ id: ind.id, name: ind.name, description: ind.description })}
